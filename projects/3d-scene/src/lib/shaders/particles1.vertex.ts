export default `#define PI 3.141592653589793
...
void main()
{
  ...
  float sineVal = sin(piTimes2 * _Time * cnoise(w_pos * _SizeFrequency));
  float tgVal = sin(sqrt(piTimes2 * _Time * cnoise(w_pos * _SizeFrequency)) * 10.0);


  vec3 world_pos = vec3((sineVal * tgVal) * 1000.0,100.0* sineVal, cnoise(w_pos * _SizeFrequency)) + w_pos;

  //vec3 world_pos = rotation3dY(5.*_Time) * w_pos;
  ...
  vec3 particle_w_pos = (modelMatrix * vec4(world_pos + offset2, 1.0)).xyz;
